...
//支付请求接口
yeePay.prototype.paybindbankcard = function(obj,cb){
	var config = this.getDefaultConfig();
	obj.merchantaccount = config.merchantaccount;
	obj.identitytype = config.identitytype;
	this._post(obj,this.config.API_TZT_paybindbankcard,cb);
}
//发送短信验证码接口
yeePay.prototype.sendvalidatecode = function(obj,cb){
	var config = this.getDefaultConfig();
	obj.merchantaccount = config.merchantaccount;
	this._post(obj,this.config.API_TZT_sendvalidatecode,cb);
}
//确认支付
yeePay.prototype.confirmvalidatecode = function(obj,cb){
	var config = this.getDefaultConfig();
	obj.merchantaccount = config.merchantaccount;
	obj.identitytype = config.identitytype;
	this._post(obj,this.config.API_TZT_confirmvalidatecode,cb);
}
//直接支付接口
yeePay.prototype.directbindpay = function(obj,cb){
	var config = this.getDefaultConfig();
	obj.merchantaccount = config.merchantaccount;
	obj.identitytype = config.identitytype;
	obj.callbackurl = config.callbackurl;
	obj.productname = config.productname;
	this._post(obj,this.config.API_TZT_directbindpay,cb);
}
//获取绑卡列表
yeePay.prototype.getbindlist = function(obj,cb){
	var config = this.getDefaultConfig();
	obj.merchantaccount = config.merchantaccount;
	obj.identitytype = config.identitytype;
	this._post(obj,this.config.API_TZT_bindlist,cb);
}
//解绑卡接口
