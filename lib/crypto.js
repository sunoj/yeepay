...
function RSAVerify(json,sign,yibaoPublickKey,algorithm){
	...
	try{
		...
		var json_values = _.values(json);
		var values = _.map(json_values, toString);
		var valStr = values.join('');
		algorithm = algorithm || "RSA-SHA1";
		var verifier = crypto.createVerify(algorithm);
		verifier.update(valStr, 'utf8');
...
function decryptKey(encryptkey,merchantPrivateKey){
	...
	try{
		...
		key = new Buffer(key, 'base64');//易宝要求必须进行base64解码才能得到正确的解密 eas key
